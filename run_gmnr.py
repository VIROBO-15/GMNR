...
from gmnr.utils import get_config, update_config_log
...
def run_exp(
    ...
    log_folder_name = "seed_{}-dist_{}-{}-torgba_cond_{}-embed_func_{}-{}".format(
        ...
        int(num_gpus),
        run_dataset,
        config.gmnr.MODEL.STYLEGAN2.torgba_cond_on_pos_enc,
        config.gmnr.MODEL.STYLEGAN2.torgba_cond_on_pos_enc_embed_func,
        cur_time,
    )
    ...
    config.gmnr.TRAIN.port = str(config.gmnr.DDP_TRAIN.port)
    config.gmnr.TRAIN.output_dir = config.LOG_DIR
    ...
    from gmnr.train import train

    mp.spawn(train, args=(num_gpus, config, master_port, run_dataset, EXP_NAME), nprocs=num_gpus, join=True)
